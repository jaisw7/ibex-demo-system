...
status_t i2c_testutils_set_speed(const dif_i2c_t *i2c, dif_i2c_speed_t speed) {
  ...
  switch (speed) {
    ...
    case kDifI2cSpeedFast:
      ...
      speed_khz = 400;
// TODO: this seems to make no difference because of the adjustments in the DIF
//speed_khz = 548;
      break;
  ...
  dif_i2c_status_t status;
  TRY(dif_i2c_get_status(i2c, &status));
LOG_INFO("got status enabled host %d\n", status.enable_host);
