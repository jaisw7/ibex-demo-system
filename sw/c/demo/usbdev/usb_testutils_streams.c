...
#include "usb_testutils_streams.h"
    ...
    if (read_method != kReadMethodNone && packet_info.length > 0) {
      ...
      switch (read_method) {
        ...
        case kReadMethodFaster: {
          ...
          usbutils_memcpy_from_mmio32(usbdev->dev->base_addr, offset, data,
                                      len);
          buf.offset += len;
...
void usb_testutils_stream_service(usb_testutils_streams_ctx_t *ctx,
                                  uint8_t id) {
  // Locate the stream context information
  CHECK(id < USBUTILS_STREAMS_MAX);
  usbdev_stream_t *s = &ctx->streams[id];
...
void usb_testutils_streams_init(usb_testutils_streams_ctx_t *ctx,
                                ...
                                usbdev_stream_flags_t flags, bool verbose) {
  CHECK(nstreams <= USBUTILS_STREAMS_MAX);
  ...
  for (unsigned id = 0U; id < nstreams; id++) {
    ...
    usb_testutils_stream_init(ctx, id, ep_in, ep_out, num_bytes, flags,
                              verbose);
  }
...
void usb_testutils_streams_service(usb_testutils_streams_ctx_t *ctx) {
  for (unsigned id = 0U; id < ctx->nstreams; id++) {
    usb_testutils_stream_service(ctx, id);
