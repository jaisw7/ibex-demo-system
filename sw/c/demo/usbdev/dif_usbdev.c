...
void test_reg(mmio_region_t base, ptrdiff_t offset, uint32_t test) {
LOG_INFO("Attempting reg write of 0x%x", test);
  // Write configuration to PHY_CONFIG register
  mmio_region_write32(base, offset, test);

LOG_INFO("Back from writing 0x%x", test);

  uint32_t read_config_val = mmio_region_read32(base, offset);
LOG_INFO("Read 0x%x", read_config_val);
}
...
#if 1
//  test_reg(usbdev->base_addr, USBDEV_PHY_CONFIG_REG_OFFSET, 0xffffu);
//  test_reg(usbdev->base_addr, USBDEV_PHY_CONFIG_REG_OFFSET, 0x0u);
 
LOG_INFO("Attempting first usbdev write");
  // Write configuration to PHY_CONFIG register
  mmio_region_write32(usbdev->base_addr, USBDEV_PHY_CONFIG_REG_OFFSET,
                      phy_config_val);

LOG_INFO("Back from writing 0x%x", phy_config_val);
#endif

LOG_INFO("Attempting read from 0x%p\n", (uint8_t*)usbdev->base_addr.base + 
USBDEV_PHY_CONFIG_REG_OFFSET);

  uint32_t read_config_val = mmio_region_read32(usbdev->base_addr, USBDEV_PHY_CONFIG_REG_OFFSET);
LOG_INFO("Read 0x%x", read_config_val);
...
void test_status_set(test_status_t test_status) {
  switch (test_status) {
    ...
    default: {
      LOG_INFO("test status 0x%x", (unsigned)test_status);
      break;
    }
  }
  while (1) {
